...
void calculateSinWave(sinewave sw, float x, float t, float y[], bool dvs, float *dydx) {
    float d = sw.k * x;
    printf("sin: %f\n", sinf(roundf(sw.k * 10000) / 10000.0 * x ));
    float temp_y = sw.a * sinf(roundf(sw.k * 10000) / 10000.0 * x);
    printf("Temp y: %f\n",temp_y);
    float temp_d = -cosf(x * sw.k  * M_PI ) * sw.w;
    for (int i = 0; i <= n; i++) {
        if (dvs){
            dydx[i] = temp_d;
        }
...
#if 0
    y = (sw.a * sin(sw.k * x + t * sw.w));
    
    vec3f* vertex;
    for (int j = 0; j <= n; j++) {
        for (int i = 0; i <= n; i++) {
            x = xStart + i * xStep;
            y = calculateY(&sw, x + animationX);
...
void drawRow(float z){
    float xStart = -1, zStart =1, xStep = 2.0f / (float)n, zStep = 2.0f / (float)n, y, x;
    z = - (z * zStep);
    ...
    for (int i = 0; i <= n; i++) {
        x = xStart + i * xStep;
        calculateSinWave(sw, x + animationX, 1, array_y, false, array);
        
        glNormal3f(-cos(x * M_PI * 2) * 0.25 * M_PI, 1.0, 0.0);
        glVertex3f(x,         array_y[i]    , actualZ);
        glVertex3f(x        , array_y[i]   , actualZ - zStep);
    }
}
...
void drawGrid(){
...
#endif
    float x, xStart = -1, xStep = 2.0f / (float)n, z = 0, zStep = 2.0f / (float)n, zStart =1;
    
    float *y_array = new float[17];
    for (int i = 0; i < 17; i++) {
        printf("Y: %f\n", y_array[i]);
    }
    float dxdy[17];
    for (int i = 0; i <= n; i++) {
        x = xStart + i * xStep;
        calculateSinWave(sw, x, 1, y_array, false, dxdy);
        for (int j = 0; j <= n; j++) {
            printf("Y: %f\n", y_array[j]);
            vec3f pos = {x, y_array[j], z};
            grid[j][i] = pos;
        }
    }
    for (int i = 0; i < 17; i++) {
    ...
    for (int i = 0; i <= 1; i++) {
        ...
        glBegin(GL_QUAD_STRIP);
        for (int j = 0; j <= n; j++) {
            glNormal3f(-cos(grid[i][j].x * M_PI * 2) * 0.25 * M_PI, 1.0, 0.0);
            
            glVertex3f(grid[i + 1][p2].x, grid[i + 1][p2].y, z);
            printf("%f, %f, %d, %d\n", grid[i + 1][p2].x, grid[i + 1][p2].y, i + 1, p2);
            glVertex3f(grid[i][p2].x, grid[i][p2].y, z);
            printf("%f, %f, %d, %d\n", grid[i][p2].x, grid[i][p2].y, i, p2);
            p2 += 1;
            next_col = !next_col;
        }
        glEnd();
